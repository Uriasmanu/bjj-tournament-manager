...
export async function POST(request: NextRequest) {
  try {
    ...
    const selectedCompetitors = competitorsData.competitors.filter(
      (c) => competitorIds.includes(c.id) && c.isActive && c.belt === belt
    );
